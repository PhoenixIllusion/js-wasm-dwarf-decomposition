...
export const SIMD = wrap(OP_SIMD);

export function parse_simd(code: number, reader: Reader): OP_Command {
  return wrap_op(code, reader, SIMD, immediates => {
    switch (code) {
      case 0x0c: //v128.const
      case 0x0d: //i8x16.shuffle
        immediates.push(...reader.getBuffer(16));
        break;
